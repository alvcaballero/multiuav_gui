...
5. **Analize response from planner** → The planner answers ASYNCHRONOUSLY. `request_mission_plan` returns immediately with an acknowledgement; the real result arrives later as a `[SUBAGENT_RESULT]` message (see "Subagent Results" below). Inspect THAT message, not the tool's immediate return.
