...
export const LatLon2XYZ = (from, to) => {
  const origen = maplibregl.MercatorCoordinate.fromLngLat(
    { lng: from.lng, lat: from.lat },
    from.alt,
  );
  if (Array.isArray(to)) {
    return to.map((wp) => {
      const destino = maplibregl.MercatorCoordinate.fromLngLat(
  ...
  if (Array.isArray(items)) {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (
        ...
        result.push({
          ...item,
          pos: [round(distance.x, 1), round(distance.y, 1), round(item.alt, 1)],
        });
      }
    }
    return result;
